...
def preparar_datos(df):
    ...
    # Columna 22 Excel = indice 21 Python
    # ==================================================
...
def tabla_html(df, tipo="examen"):
    ...
    if tipo == "examen":
        ...
        mostrar["FECHA PROXIMO EXAMEN"] = (
            mostrar["FECHA PROXIMO EXAMEN"]
            .dt.strftime("%Y-%m-%d")
        )

        mostrar.rename(
            columns={"DIAS_EXAMEN": "DIAS_RESTANTES"},
            inplace=True
        )
        ...
        mostrar["FECHA ACTUALIZACION ALTURAS"] = (
            mostrar["FECHA ACTUALIZACION ALTURAS"]
            .dt.strftime("%Y-%m-%d")
        )

        mostrar.rename(
            columns={"DIAS_ALTURAS": "DIAS_RESTANTES"},
            inplace=True
        )
...
def enviar_correo(pe, pre, ce, pa, pra, ca):
    ...
    <body style='font-family:Arial,sans-serif;'>
        ...
        <h2>🩺 RESUMEN GENERAL EXÁMENES OCUPACIONALES</h2>

        <table style='width:100%;text-align:center;border-collapse:collapse;'>

            <tr>

                <td style='background:#d4edda;padding:12px;border:1px solid #ccc;'>

                    <b>🟢 Preventivos</b><br>{len(pe)}

                </td>

                <td style='background:#fff3cd;padding:12px;border:1px solid #ccc;'>

                    <b>🟡 Prioritarios</b><br>{len(pre)}

                </td>

                <td style='background:#f8d7da;padding:12px;border:1px solid #ccc;'>

                    <b>🔴 Críticos/Vencidos</b><br>{len(ce)}
        ...
        <h2>⛑️ RESUMEN GENERAL CERTIFICADO DE ALTURAS</h2>
        ...
        <table style='width:100%;text-align:center;border-collapse:collapse;'>
            ...
            <tr>
                ...
                <td style='background:#d4edda;padding:12px;border:1px solid #ccc;'>
                    ...
                    <b>🟢 Preventivos</b><br>{len(pa)}

                </td>

                <td style='background:#fff3cd;padding:12px;border:1px solid #ccc;'>

                    <b>🟡 Prioritarios</b><br>{len(pra)}

                </td>

                <td style='background:#f8d7da;padding:12px;border:1px solid #ccc;'>

                    <b>🔴 Críticos/Vencidos</b><br>{len(ca)}
        ...
        <h2>🩺 TABLAS DETALLADAS EXÁMENES OCUPACIONALES</h2>

        <h3 style='color:green;'>
            🟢 PERSONAL EN ALERTA PREVENTIVA EXÁMENES
        </h3>

        {tabla_html(pe, "examen")}

        <h3 style='color:#b8860b;'>
            🟡 PERSONAL EN ALERTA PRIORITARIA EXÁMENES
        </h3>

        {tabla_html(pre, "examen")}

        <h3 style='color:red;'>
            🔴 PERSONAL CRÍTICO / EXÁMENES VENCIDOS
        </h3>
        ...
        <h2>⛑️ TABLAS DETALLADAS CERTIFICADO DE ALTURAS</h2>

        <h3 style='color:green;'>
            🟢 PERSONAL EN ALERTA PREVENTIVA ALTURAS
        </h3>

        {tabla_html(pa, "alturas")}

        <h3 style='color:#b8860b;'>
            🟡 PERSONAL EN ALERTA PRIORITARIA ALTURAS
        </h3>

        {tabla_html(pra, "alturas")}

        <h3 style='color:red;'>
            🔴 PERSONAL CRÍTICO / CERTIFICADO VENCIDO ALTURAS
        </h3>
        ...
        <p style='font-size:12px;color:gray;'>
            ...
            Sistema de Vigilancia Documental SST - VALSER.
    ...
    msg["Subject"] = (
        "🔔 Alerta SST - "
        "Exámenes Ocupacionales y Certificados de Alturas"
    )
...
def main():
    ...
    try:
        ...
        # EXAMENES
        pe, pre, ce = clasificar_alertas(
            df,
            "DIAS_EXAMEN"
        )

        # ALTURAS
        pa, pra, ca = clasificar_alertas(
            df,
            "DIAS_ALTURAS"
        )
